...
public class LikeDao {
	...
	public boolean insertLike(int pid, int uid) {
		...
		try {
			...
			String q1 = "select * from liked where uid=? and pid=?";
			PreparedStatement ps1 = this.con.prepareStatement(q1);
			ps1.setInt(1, uid);
			ps1.setInt(2, pid);
			ResultSet e=ps1.executeQuery();
			if(e.next()) {
				String q2 = "delete from liked where uid=? and pid=?";
				PreparedStatement ps2 = this.con.prepareStatement(q1);
				ps2.executeUpdate();
